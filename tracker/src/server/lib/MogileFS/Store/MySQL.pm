...
# XXX: 扩展domain字段宽度，以便适应以大量domain隔离数据的情形
sub upgrade_expand_domain_width
{
	my $self=shift;
	$self->dowell("ALTER TABLE domain MODIFY COLUMN dmid INT UNSIGNED NOT NULL");
	$self->dowell("ALTER TABLE class MODIFY COLUMN dmid INT UNSIGNED NOT NULL");
	$self->dowell("ALTER TABLE file MODIFY COLUMN dmid INT UNSIGNED NOT NULL");
	$self->dowell("ALTER TABLE tempfile MODIFY COLUMN dmid INT UNSIGNED NOT NULL");
	$self->dowell("ALTER TABLE file_to_replicate MODIFY COLUMN fromdevid MEDIUMINT UNSIGNED");
}

# XXX: 增加mimetype字段记录每个文件的默认类型，以方便Web应用
sub upgrade_add_file_mimetype
{
	my $self=shift;
	unless($self->column_type("file","mimetype")) {
		# mimetype的默认值NULL代表"application/octet-stream"，即普通二进制数据文件
		$self->dowell("ALTER TABLE file ADD COLUMN mimetype VARCHAR(255) DEFAULT NULL");
	}
}

# XXX: 增加createtime字段记录每个文件的创建时戳，以便管理
sub upgrade_add_file_createtime
{
	my $self=shift;
	unless($self->column_type("file","createtime")) {
		$self->dowell("ALTER TABLE file ADD COLUMN createtime INT UNSIGNED NOT NULL DEFAULT 0");
	}
}
...
sub pre_daemonize_checks {
    ...
    my $id = eval {
		# XXX: YLEM中domain数量可能很大，而有效dmid总是>0，因此这里将DBI检测记录的
		# dmid从99改为0，以免产生domain冲突。
#        $self->register_tempfile(dmid => 99,
        $self->register_tempfile(dmid => 0,
