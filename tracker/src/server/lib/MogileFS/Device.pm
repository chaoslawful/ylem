...
sub vivify_directories {
    ...
    #    http://10.0.0.26:7500/dev2/vol0/0/000/148/0000148056.fid

    # four directories we'll want to make:
	#    http://10.0.0.26:7500/dev2/vol0
    #    http://10.0.0.26:7500/dev2/vol0/0
    #    http://10.0.0.26:7500/dev2/vol0/0/000
    #    http://10.0.0.26:7500/dev2/vol0/0/000/148

    croak "non-HTTP mode no longer supported" unless $path =~ /^http/;
    return 0 unless $path =~ m!/dev(\d+)/vol(\d+)/(\d+)/(\d\d\d)/(\d\d\d)/\d+\.fid$!;
    my ($devid, $volid, $p1, $p2, $p3) = ($1, $2, $3, $4, $5);

    my $dev = MogileFS::Device->of_devid($devid);
    return 0 unless $dev->exists;

	# XXX: 假定存储设备上卷目录必须同设备目录一同被手工创建好，因此这里不再自动创建卷目录
#	$dev->create_directory("/dev$devid/vol$volid");
    $dev->create_directory("/dev$devid/vol$volid/$p1");
    $dev->create_directory("/dev$devid/vol$volid/$p1/$p2");
    $dev->create_directory("/dev$devid/vol$volid/$p1/$p2/$p3");
}
...
sub create_directory {
    ...
    my $sock = IO::Socket::INET->new(PeerAddr => $peer, Timeout => 60)
        or return 0;
	...
	# XXX: 注意，这里后台WebDAV服务器的返回值可能为405（Method Not Allowed），
	# 出现该返回值时，可能是后端WebDAV服务器不支持MKCOL命令，或者是MKCOL命令
	# 参数所代表的资源已经存在。对于MogileFS来说，它的目的是尽可能支持多样化
	# 的后端存储服务，因此不能假定405错误代表资源已存在，为了保险起见只能认为
	# 后端WebDAV服务不支持MKCOL命令；但对于YLEM系统来说，后端使用何种WebDAV
	# 服务是完全确定的（Lighttpd或Nginx），它们肯定都支持MKCOL命令，所以此处
	# 可以安全地假定405错误代表的是资源已存在。
