...
# TODO:
# * 增加一个新的Tracker命令或为DELETE增加一个参数，实现文件的逻辑删除，
# 以期在特定时间段内恢复之。需要修改file表，增加删除时戳和删除标志位。
...
use constant FEED_WATCHDOG_INTERVAL => 5; 	# 喂狗时间间隔
...
sub work {
    ...
    while (1) {
        my $rout;
        unless (select($rout=$rin, undef, undef, FEED_WATCHDOG_INTERVAL)) {
            $self->still_alive;
            next;
        }
...
sub cmd_create_open {
	...
	# 除非定义了全局挂钩cmd_create_open_order_devices，否则就基于每个设备的存储空间空闲比例进行排序，
	# 空余比例越大的存储设备越有可能排名靠前（最终结果是一个随机序列）
...
# 根据存储空间空闲比例对给定设备列表排序，空闲度越高的设备排名越靠前
...
sub cmd_create_close {
    ...
    unless (defined $key && length($key)) {
		# XXX: 当关闭文件没有指定key时，我们认为是要删除掉临时文件元数据信息及
		# 对应的文件内容，但MogileFS的原始代码在这里忘了将Client给出的文件内容
		# 存储位置同临时文件对应的fid关联起来，产生了只有临时文件元数据信息被删
		# 除，而对应的文件内容却没有被删掉的问题；在YLEM系统中我们必须要注意防止
		# 这种情况发生，因此首先要将fid对应的文件位置保存到file_on数据表里，再使用
		# 普通的文件删除数据库操作。
		$dfid->add_to_db;
		# 删掉临时文件fid对应的元数据行，并将fid添加到file_to_delete表中
		# 等待Delete进程删除文件内容。
        $fid->delete;
        return $self->ok_line;
    }
...
sub cmd_get_paths {
	...
	# 除非定义了全局挂钩cmd_get_paths_order_devices，否则就按照设备I/O使用率进行排序，
	# I/O使用率越低的设备排名越有可能靠前（最终结果是一个随机序列）
	...
	# XXX: 在返回结果中添加默认MIME type
	my $mimetype=$fid->mimetype;
	# 若默认MIME type不为空，则返回之，否则返回application/octet-stream作为默认类型
	$ret->{mimetype} = (defined($mimetype) && length($mimetype)) ? $mimetype : "application/octet-stream";

    return $self->ok_line($ret);
}

# 按照监测到的设备I/O使用率对给定的设备列表进行排序，使用率越低的设备排名越有可靠前
...
sub cmd_stats {
    ...
    my $sto = Mgd::get_store();
    my $dbh;
	# XXX: 使用slave DB句柄以降低master DB压力
	$sto->slaves_ok(sub {$dbh=$sto->read_store()->dbh;})
	...
	# XXX: 考虑到某个domain可能只用了default class，因此class数据表中就没有对应domain的记录，
	# 在这种情况下原来的SQL语句将忽略这个domain，这也许并不是我们想要的结果。更改后的SQL语句
	# 会正常显示只用到default class的domain。
    $rows = $dbh->selectall_arrayref('SELECT domain.dmid, namespace, classid, classname ' .
                                     'FROM domain LEFT JOIN class ON class.dmid = domain.dmid');
    foreach my $row (@$rows) {
        $classes{$row->[0]}->{name} = $row->[1];
		# XXX: 没有对应domain下的class记录时这里的classid为空，可以用0代替
        $classes{$row->[0]}->{classes}->{$row->[2]||0} = $row->[3];
    }
    ...
    if ($args->{replication} || $args->{all}) {
		...
		# XXX: 用slave DB来获取统计数据，避免增加master DB的压力
        my @stats;
		$sto->slaves_ok(sub {@stats=$sto->read_store()->get_stats_files_per_devcount;});
