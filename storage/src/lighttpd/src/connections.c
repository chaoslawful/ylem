...
#define UPLOAD_USING_TEMPFILE_THRESHOLD (1024*1024)
#define UPLOAD_TEMPFILE_MAXLENGTH (16*1024*1024)
			if (con->request.content_length > UPLOAD_USING_TEMPFILE_THRESHOLD) {
				chunk *dst_c = NULL;
				...
				if (dst_cq->last &&
					...
					/* ok, take the last chunk for our job */
			 		...
			 		if (dst_cq->last->file.length < UPLOAD_TEMPFILE_MAXLENGTH) {
						dst_c = dst_cq->last;
