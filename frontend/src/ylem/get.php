...
/*
	...
	8. 若找到可访问的存储位置则发送响应头及文件内容；否则根据有无redirect参数决定是重定向还是直接403
	Forbidden错误
...
// 解密下载文件信息密文
$plain=$secure->decrypt(base64_decode_urlsafe($enc));

// 将下载文件信息明文转换为键值数组
$params=array();
$pairs=explode("\002",$plain);
foreach($pairs as $pair) {
	list($k,$v)=explode("\001",$pair);
	$params[$k]=$v;
}
...
$ct_type=isset($params["ct"])?$params["ct"]:false; 		// 选填参数，指定header中的Content-Type内容。
$filename=isset($params["fn"])?$params["fn"]:false; 	// 选填参数，指定header中Content-Disposition里的filename，
							// 不填则默认为文件id加上可识别的Content-Type后缀。
$redirect=isset($params["rd"])?$params["rd"]:false; 	// 选填参数，若无法获取指定文件则重定向到该URL，
							// 不填则在无法获取文件时报告404错误
...
if(isset($_SERVER['HTTP_REFERER'])) {
	...
	if(!preg_match('/^(?:https?:\/\/[^\/]*(yahoo\.com|yahoo\.cn|koubei\.com)(:[0-9]+)?\/.*)?$/',$referer)) {
		// 不满足对HTTP_REFERER的模式要求
		if($ns == "sns_share_photo") {
			// 为SNS域的图片增加访问例外
			$continue=true;
		} else {
			// 阻止其他域的图片被访问
