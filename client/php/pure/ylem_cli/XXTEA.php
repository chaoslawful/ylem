...
class Crypt_XXTEA {
    ...
    function setKey($key) {
        if (is_string($key)) {
            ...
            error_log('The secret key must be a string or long integer array.');
            return false;
        }
        if (count($k) > 4) {
            error_log('The secret key cannot be more than 16 characters or 4 long values.');
            return false;
        } elseif (count($k) == 0) {
            error_log('The secret key cannot be empty.');
            return false;
        } elseif (count($k) < 4) {
            for ($i = count($k); $i < 4; $i++) {
                $k[$i] = 0;
    ...
    function encrypt($plaintext) {
        if ($this->_key == null) {
            error_log('Secret key is undefined.');
            return false;
        }
        if (is_string($plaintext)) {
            ...
            error_log('The plain text must be a string or long integer array.');
            return false;
        }
    }
    ...
    function decrypt($chipertext) {
        ...
        if (is_string($chipertext)) {
            ...
            error_log('The chiper text must be a string or long integer array.');
            return false;
        }
    }
